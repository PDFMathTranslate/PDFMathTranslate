...
| **MiniMax**         | `minimax`      | `MINIMAX_API_KEY`, `MINIMAX_MODEL`, `MINIMAX_BASE_URL` (선택)         | `[Your MINIMAX_API_KEY]`, `MiniMax-M3`, `https://api.minimax.io/v1` | [MiniMax](https://platform.minimaxi.com/) 참조. 기본값은 `MiniMax-M3`이며, 이전 모델은 `MINIMAX_MODEL=MiniMax-M2.7`로 지정합니다. 중국 엔드포인트는 `MINIMAX_BASE_URL=https://api.minimaxi.com/v1`로 설정합니다.                                                          |
